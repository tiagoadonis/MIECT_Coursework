...
public class TestFractions {
  ...
  public static void main(String[] args) {
    ...
    // Ausência de comutatividade na subtração e na divisão
    assert !(x.subtract(y).equals ( y.subtract(x) )) : "x-y != y-x";
    assert !(x.divide(y).equals ( y.divide(x) )) : "x/y != y/x";
    
    // Associatividade da multiplicação e da adição
    assert (x.multiply(y)).multiply(z).equals( (y.multiply(x)).multiply(z) ) : "x*(y*z) == y*(x*z))";
    assert (x.add(y)).add(z).equals ( y.add(x).add(z) ) : "x+(y+z) == y+(x+z)";
    
    // Distributividade da multiplicação
    assert (x.add(y)).multiply(z).equals( (x.multiply(z)).add(y.multiply(z)) ) : "(x+y)*z == (x*z)+(y*z)";
