...
public class TestSorting{
	static public void main(String[] args) {
		...
		for(int i = 0; i < args.length; i++){
			...
			arrays[0] = randomArray(j, 2*j);  	// args.length random numbers, in random order
			...
			for(int k = 0; k < 3; k++) {
				for (int m = 0; m < algorithm.length; m++) {
					int[] aCopy = arrays[k].clone();
